#!/usr/bin/env python3
"""
Run ASR on Weibo2014 (10 subjects) and report accuracy vs baseline.

Same evaluation as for Cho2017: tangent-space + 5-fold CV.
Requires: data/weibo2014/processed (prepared like in your existing pipeline).
Requires: asrpy installed in the environment (pip install asrpy).

Usage:
    cd /path/to/comparison
    python scripts/run_asr_weibo2014.py            # all 10 subjects
    python scripts/run_asr_weibo2014.py --smoke    # subject 1 only
"""
...
DATA_ROOT = PROJECT_ROOT / "data" / "weibo2014" / "processed"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run ASR on Weibo2014 and compare with baseline.")
    ...
    if not DATA_ROOT.exists():
        print(f"Error: {DATA_ROOT} not found. Prepare Weibo2014 first (see existing scripts).")
        sys.exit(1)
    ...
    print("=" * sep_len)
    print("\nCompare this ASR row with your GEDAI results on Weibo2014.")
